...
if(isset($_POST['submit'])) {
	...
	if(empty($error)) {
		//OK
		$result = mysql_query( "SELECT * FROM persons WHERE contact='$contact' AND address='$address' AND bloodgroup='$bloodgroup'") or die(mysql_error());
		if(mysql_num_rows($result) == 0) {
			...
			if(!$result2) {
				die('Could not add information !! Sorry for inconvinence.');
			}else {
				header('Location: add_entry.php');
			}
		}else {
			echo '<span class="error">
				Following person information is already present in Database. Please try again.	
				</span>
			';
		}
	}else {
		...
		<section id="right_side">
			<form id="form" class="container" method="post" action="add_entry.php" enctype="multipart/form-data">
					...
					<div class="image_field">
						<label for="userpic">Upload Image:</label>
						<input type="file" class="input" id="userpic" name="userpic"/>
					</div>
					...
					<div class="field">
						...
						<input type="text" class="input" id="name" name="name" maxlength="50"/>
					</div>
					<div class="field">
